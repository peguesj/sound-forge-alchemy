defmodule SoundForgeWeb.DawTabTest do
  ...
  describe "DAW component events" do
    ...
    test "pick_track loads track into DAW", %{conn: conn, track: track} do
      ...
      html =
        view
        |> element("#daw-tab [phx-click='pick_track'][phx-value-track-id='#{track.id}']")
        |> render_click()
    ...
    test "select_operation after picking track", %{conn: conn, track: track} do
      ...
      html =
        view
        |> element("#daw-tab [phx-click='select_operation'][phx-value-type='crop']")
        |> render_click()
    ...
    test "select_stem after picking track", %{conn: conn, track: track, stem: stem} do
      ...
      html =
        view
        |> element("#daw-tab [phx-click='select_stem'][phx-value-stem-id='#{stem.id}']")
        |> render_click()
    ...
    test "select_operation trim", %{conn: conn, track: track} do
      ...
      result =
        try do
          view
          |> element("#daw-tab [phx-click='select_operation'][phx-value-type='trim']")
          |> render_click()
        rescue
    ...
    test "select_operation fade_in", %{conn: conn, track: track} do
      ...
      result =
        try do
          view
          |> element("#daw-tab [phx-click='select_operation'][phx-value-type='fade_in']")
          |> render_click()
        rescue
          ArgumentError -> :not_found
        end
    ...
    test "select_operation fade_out", %{conn: conn, track: track} do
      ...
      result =
        try do
          view
          |> element("#daw-tab [phx-click='select_operation'][phx-value-type='fade_out']")
          |> render_click()
        rescue
          ArgumentError -> :not_found
        end
    ...
    test "select_operation split", %{conn: conn, track: track} do
      ...
      result =
        try do
          view
          |> element("#daw-tab [phx-click='select_operation'][phx-value-type='split']")
          |> render_click()
        rescue
          ArgumentError -> :not_found
        end
    ...
    test "select_operation gain", %{conn: conn, track: track} do
      ...
      result =
        try do
          view
          |> element("#daw-tab [phx-click='select_operation'][phx-value-type='gain']")
          |> render_click()
        rescue
          ArgumentError -> :not_found
        end
  ...
  describe "module" do
    ...
    test "exports handle_event/3" do
      assert {:handle_event, 3} in SoundForgeWeb.Live.Components.DawTabComponent.__info__(
               :functions
             )
    end
