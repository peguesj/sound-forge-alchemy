defmodule SoundForge.Jobs.WorkersCoverageTest do
  ...
  setup do
    ...
    track = track_fixture(%{user_id: user.id, title: "Worker Test", artist: "Test", duration: 100})
    ...
    test "perform with nonexistent file raises", %{track: track, pj: pj} do
      job = %Oban.Job{
        args: %{
          "track_id" => track.id,
          "job_id" => pj.id,
          "file_path" => "priv/uploads/downloads/nonexistent_#{System.unique_integer([:positive])}.mp3"
        },
        attempt: 1,
        id: 1,
  ...
  describe "MultiStemWorker" do
    test "new/1 creates valid changeset" do
      cs = MultiStemWorker.new(%{
        "track_id" => "t1", "job_id" => "j1",
        "file_path" => "/tmp/x.mp3", "stem_list" => ["vocals", "drums"]
      })
      assert cs.valid?
    end
  ...
  describe "VoiceCleanWorker" do
    test "new/1 creates valid changeset" do
      cs = VoiceCleanWorker.new(%{"track_id" => "t1", "job_id" => "j1", "file_path" => "/tmp/x.mp3"})
      assert cs.valid?
    end
  ...
  describe "VoiceChangeWorker" do
    test "new/1 creates valid changeset" do
      cs = VoiceChangeWorker.new(%{"track_id" => "t1", "job_id" => "j1", "file_path" => "/tmp/x.mp3"})
      assert cs.valid?
    end
  ...
  describe "AutoCueWorker" do
    ...
    test "perform with track that has no download", %{track: _track, user: user} do
      ...
      track2 = track_fixture(%{user_id: user.id, title: "No Download", artist: "Test", duration: 50})
      job = %Oban.Job{
        args: %{
          "track_id" => track2.id,
          "user_id" => user.id
