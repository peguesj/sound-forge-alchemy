defmodule SoundForge.Jobs.VoiceCleanWorker do
  @moduledoc """
  Oban worker for cloud-based voice cleaning using the lalal.ai API.

  Uploads a track's audio file to lalal.ai, initiates a voice-clean split
  (vocal isolation with noise cancelling), polls for completion with
  exponential backoff, downloads the resulting stems, creates Stem records,
  and chains the AnalysisWorker on successful completion.

  Voice cleaning produces two stems:
  - **stem** (clean voice): mapped to `:vocals`
  - **back** (noise back track): mapped to `:other`
  ...
  ## Job Arguments
    ...
    - `"noise_cancelling_level"` - Noise cancelling intensity: 0 (none), 1 (medium), 2 (aggressive)
  ...
  def perform(%Oban.Job{
    ...
    noise_cancelling_level = Map.get(args, "noise_cancelling_level", 0)
    splitter = Map.get(args, "splitter", "phoenix")
    dereverb = Map.get(args, "dereverb", false)
    encoder_format = Map.get(args, "encoder_format", "wav")

    Logger.info(
      "Starting lalal.ai voice clean: noise_level=#{noise_cancelling_level}, splitter=#{splitter}"
    )
    ...
    with {:ok, source_id} <- LalalAI.upload_source(resolved_path),
      ...
      {:error, reason} ->
        ...
        Logger.error("lalal.ai voice clean failed: #{error_msg}")
        fresh_job = Music.get_processing_job!(job_id)
        Music.update_processing_job(fresh_job, %{status: :failed, error: error_msg})
        PipelineBroadcaster.broadcast_stage_failed(track_id, job_id, :processing)
        {:error, error_msg}
  ...
  defp poll_until_complete(task_id, job_id, track_id, attempt, interval) do
    ...
    case LalalAI.get_status(task_id) do
      {:ok, %{status: "success", stem: stem, accompaniment: _accompaniment}} ->
        Logger.info("lalal.ai voice_clean task #{task_id} completed successfully")
      ...
      {:ok, %{status: "queued"}} ->
        Logger.debug("lalal.ai voice_clean task #{task_id} still queued (attempt #{attempt})")
        next_interval = min(interval * 2, @max_poll_interval_ms)
        poll_until_complete(task_id, job_id, track_id, attempt + 1, next_interval)

      {:ok, %{status: "error", error: error_message}} ->
        Logger.error("lalal.ai voice_clean task #{task_id} failed: #{error_message}")
        {:error, {:lalalai_error, error_message}}

      {:ok, %{status: unknown_status}} ->
        Logger.warning("lalal.ai voice_clean task #{task_id} unknown status: #{unknown_status}")
        next_interval = min(interval * 2, @max_poll_interval_ms)
        poll_until_complete(task_id, job_id, track_id, attempt + 1, next_interval)
  ...
  defp process_completed_stems(track_id, job_id, file_path, stem_urls, noise_cancelling_level) do
    stem_dir = build_stem_dir(track_id)
    File.mkdir_p!(stem_dir)

    stem_records = []

    # Download the clean voice stem (primary stem)
    stem_records =
      case Map.get(stem_urls, :stem) do
        %{"link" => download_url} when is_binary(download_url) ->
          stem_path = Path.join(stem_dir, "voice_clean.wav")

          case LalalAI.download_stem(download_url, stem_path) do
            {:ok, saved_path} ->
              relative_path = make_relative(saved_path)
              stem_records ++ persist_stem(track_id, job_id, :vocals, saved_path, relative_path)

            {:error, reason} ->
              Logger.error("Failed to download clean voice stem: #{inspect(reason)}")
              stem_records
          end

        _ ->
          Logger.warning("No download link in lalal.ai voice_clean stem result")
          stem_records
      end

    # Download the noise back track (accompaniment/back)
    # The get_status response maps the back track to the :stem key as well,
    # but for voice_clean the API returns stem (voice) and back (noise).
    # The parse_status_response puts "stem" -> :stem and "accompaniment" -> not present
    # for voice_clean. Check the raw stem_urls map for back track info.
    stem_records =
      case get_back_track_url(stem_urls) do
        {:ok, back_url} ->
          back_path = Path.join(stem_dir, "noise_back.wav")

          case LalalAI.download_stem(back_url, back_path) do
            {:ok, saved_path} ->
              relative_path = make_relative(saved_path)
              stem_records ++ persist_stem(track_id, job_id, :other, saved_path, relative_path)

            {:error, reason} ->
              Logger.error("Failed to download noise back track: #{inspect(reason)}")
              stem_records
          end

        :none ->
          Logger.debug("No back track URL in voice_clean result")
          stem_records
      end
    ...
    Logger.info("lalal.ai voice clean complete, stems=#{length(stem_records)}")
  ...
  # Extract back track URL from the stem_urls map.
  ...
  defp get_back_track_url(_), do: :none
