defmodule SoundForgeWeb.Live.Components.MidiOscStatusBar do
  ...
  def status_bar(assigns) do
    ...
    <!-- Activity rate -->
      <div
        :if={@message_rate > 0}
        class="flex items-center gap-1 ml-auto text-gray-600"
        title="Messages/sec"
      >
        <div class="flex items-end gap-px h-3">
          <div
            :for={i <- 1..5}
            class={"w-0.5 rounded-t transition-all " <>
      ...
      >
        <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path
            stroke-linecap="round"
            stroke-linejoin="round"
            d={if @collapsed, do: "M19 9l-7 7-7-7", else: "M5 15l7-7 7 7"}
          />
        </svg>
      </button>
    </div>
